...
//!
//! - `HeaderCondition` matches if the named header matches the given pattern.
//! - `PathCondition` matches if the Request path matches the given pattern.
//! - `ExistenceCondition` matches if the Request path resolves to a real file.
//! - `NonExistenceCondition` matches if the Request path does not resolve.
//!
//! In addition to these core types, any function with a `Fn(&Request) -> bool`
//! signature may also be used anywhere a `Condition` is expected. This allows
//! any arbitrary logic to be applied to decide a match. Because a `Request`
...
//! ```
//!
//! Multiple Conditions may be grouped together to form logical conditions
//! using `condition.and(other)` or `condition.or(other)` to apply conditions
//! with AND or OR logic respectively.
//!
//! # Rewriters
//!
//! There are several types of `Rewriter` for rewriting Request state:
//!
//! - `HeaderRewriter` rewrites named header with given pattern and replacement.
//! - `PathRewriter` rewrites Request path with given pattern and replacement.
//!
//! As with `Condition`, any function with a `Fn(Request) -> Request` signature
//! may also be used anywhere a `Rewriter` is accepted. This allows any custom
//! logic to be used to produce a rewritten `Request`. Because a `Request` may
