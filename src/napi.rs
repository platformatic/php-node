...
use crate::{Embed, EmbedRequestError, Handler, Request, RequestRewriter, Response};
  ...
  fn compute(&mut self) -> Result<Self::Output> {
    ...
    if !self.throw_request_errors {
      result = result.or_else(|err| {
        Ok(match err {
          EmbedRequestError::ScriptNotFound(_script_name) => http_handler::response::Builder::new()
            .status(404)
            .body(bytes::BytesMut::from("Not Found"))
            .unwrap(),
          _ => http_handler::response::Builder::new()
            .status(500)
